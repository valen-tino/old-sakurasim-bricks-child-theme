...
function customise_checkout_field($checkout) {
    ...
    woocommerce_form_field('monthlyfee_payment_option', array(
        ...
        'options'       => array(
            'blank' => 'Please select an option',
            'smartpit' => 'Convenience Store Via Smartpit',
            'credit-card-stripe' => 'Credit Card Via Stripe',
            'bank-transfer-gmo' => 'Bank Transfer Via GMO',
...
function add_custom_datepicker_script()
{
    ...
    selectMonthlyFeeOption.addEventListener("change", function() {
        ...
        switch (selectedOption) {
            case 'credit-card-stripe':
                priceChangeMessage = 'Congratuations, You got a ¥30 discount for choosing Credit Card.';
                priceChangeAmount = 30;
                methodChange = 'Credit Card Via Stripe (Discount ¥30)';
                break;
            case 'smartpit':
                priceChangeMessage = '';
                priceChangeAmount = 0;
                methodChange = 'Convenience Store Via Smartpit';
                break;
            case 'bank-transfer-gmo':
                priceChangeMessage = 'Congratuations, You got a ¥180 discount for choosing Bank Transfer.';
                priceChangeAmount = 180;
                methodChange = 'Bank Transfer Via GMO (Discount ¥180)';
                break;
            case 'choose-later':
    ...
    if (methodChange != ''){
        ...
        priceChangeElement.textContent = priceChangeMessage;
...
function display_custom_order_meta_in_checkout_success($order_id) {
    ...
    if ($final) {
        ...
        echo '<h3>How to pay the monthly fee Via '; 
            ...
            function updatePaymentContent(method) {
                ...
                switch(method) {
                    case 'Credit Card (Stripe)':
                        ...
                        break;
                    case 'Convenience Store (Smartpit)':
                    ...
                    case 'Bank Transfer (GMO)':
                        ...
                        break;
                    case 'Choose Later':
                        paymentContent = 'Content for Choose Later payment method';
                        break;
                    default:
                        paymentContent = '';
                }
...
function add_custom_order_item_row($total_rows, $order, $tax_display) {
    ...
    foreach ($total_rows as $key => $value) {
        if ($key === 'order_total') {
            ...
            if ($custom_data === 'credit-card-stripe' || $custom_data === 'bank-transfer-gmo') {
...
function custom_email_header( $email_heading, $email ) {
    // Get the image ID from the WordPress Customizer or any other source
    $image_id = 680;
    
    // Get the URL of the image
    $image_url = wp_get_attachment_image_src( $image_id, 'full' )[0]; // 'full' is the image size
    
    // Output the image in the email header
    if ( $image_url ) {
        echo '<img src="' . esc_url( $image_url ) . '" alt="Sakura SIM Logo" style="width: 100%; max-width: 150px; height: auto;" />';
    }
}